...
# ✅ استيراد ImportValidationService
...
class ContractStructureMigrationService:
    ...
    # ============================================================
    # ✅ _get_or_create_package - دالة مستقلة للبحث والإنشاء
    # ============================================================
    @staticmethod
    def _get_or_create_package(
        ...
        """
        Find package from cache or create it.
        """
    ...
    def migrate(dataframe):
        ...
        with transaction.atomic():
            ...
            # ✅ ✅ ✅ Cache للباكدجات - تحميل كامل بدون .only()
            ...
            for chunk_start in range(0, total_rows, chunk_size):
                ...
                for row in chunk_data:
                    ...
                    # ============================================================
                    # ✅ Package Lookup - استخدام الدالة المستقلة
                    # ============================================================
                    ...
                    if contract_package is None:
                        if ContractPackage.objects.filter(
                            contract=contract,
                            package=package,
                        ).exists():
                            print(
                                f"⚠ Cache Miss: contract={contract.id}, package={package.id}"
                            )
                    ...
                    if len(contract_packages_to_create) >= 500:
                        ...
                        # Refresh cache for newly created rows
                        created_keys = {
                            (cp.contract_id, cp.package_id)
                            for cp in contract_packages_to_create
                        }

                        for cp in ContractPackage.objects.filter(
                            contract_id__in=[k[0] for k in created_keys],
                            package_id__in=[k[1] for k in created_keys],
                        ):
                            contract_packages_cache[
                                (cp.contract_id, cp.package_id)
                            ] = cp
